...
bot.on('message', m => {
    ...
    if (checklevel(m) == 0) {
        ...
        /*if (blacklist(m)) {
         m.delete();
         return m.reply("Pas d'insultes !").then(m => {
         m.delete(3000)
         }).catch(console.error);
         }*/
        if (capsLimit(m)) {
...
var commands = {
        ...
        'serverinfo': {
            ...
            level: 0,
            exec: function (message) {
                let embed = new Discord.RichEmbed().setTitle("Information sur le serveur Discord ").setURL("https://discordapp.com/invite/HxNdn").setThumbnail(message.guild.iconURL)
        ...
        'annonce': {
            ...
            exec: function (message) {
               ...
               if (!channel) return message.reply("Le channel ne semble pas valide")
                    const embed = new Discord.RichEmbed().setTitle("__**ANNONCE**__").setColor("#FFFF00")
        ...
        ,
        'alerte': {
            description: 'Permet d\'effectuer une alerte dans un canal textuel',
            syntax: config.prefix + 'alerte [#channel] <message>',
            level: 1,
            exec: function (message) {
                let args = message.content.split(" ").slice(1);
                let alerte;
                if (message.mentions.channels.size === 0) {
                    ...
                    channel = message.mentions.channels.first()
                    alerte = args.splice(1).join(" ");
                }
               if (!channel) return message.reply("Le channel ne semble pas valide")
                    const embed = new Discord.RichEmbed().setTitle("__**ALERTE**__").setColor("#F00")
                        embed.setDescription(alerte)    
                channel.sendEmbed(embed)
...
function checklevel(message) {
    ...
    let level = 0;
    message.member.roles.array().forEach(r => {
        if (r.position > level) {
            level = r.position
        }
...
function blacklist(m) {
    let ww = m.content.toLowerCase().split(" ");
    let flag = false;
    ww.forEach((w, c) => {
        list.forEach(ww => {
            if (ww === w) {
